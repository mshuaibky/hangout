...
import {logoutAdmin} from '../../helpers/adminHelpers'

function Sidebar() {
  const id = sessionStorage.getItem('adminId');
  const navigate=useNavigate()
  const handleLogout=()=>{
    logoutAdmin(id).then((data)=>{
      if(data){
      ...
      <div>
        ...
        <ul className="mt-6 space-y-2 tracking-wide">
          <li className="min-w-max">
            <a href="/admin/dashboard" aria-label="dashboard" className="relative flex items-center space-x-4 bg-gradient-to-r from-sky-600 to-cyan-400 px-4 py-3 text-white">
          ...
          <li className="min-w-max">
            ...
            <a href="#" className="bg group flex items-center space-x-4 rounded-full px-4 py-3 text-gray-600">
              ...
              <span className="group-hover:text-gray-700">Owners</span>
            </a>
            </Link>
          </li>
          <li className="min-w-max">
          ...
          <li className="min-w-max">
            <a className="group flex items-center space-x-4 rounded-md px-4 py-3 text-gray-600">
              ...
              <span className="group-hover:text-gray-700">Restaurants</span>
            </a>
          </li>
          </Link>
          <li className="min-w-max">
            <a href="/admin/sales-report" className="group flex items-center space-x-4 rounded-md px-4 py-3 text-gray-600">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
